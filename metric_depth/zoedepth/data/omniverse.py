...
class OmniverseObject(Dataset):
    ...
    def __init__(self, data_dir, split = 'test', **kwargs):
        ...
        print(self.data_dir,os.path.join(self.data_dir, '*.h5'))
        full_h5_paths = sorted(glob(os.path.join(self.data_dir, '*.h5')))
        print(f"Found {len(full_h5_paths)} h5 files.")
...
def get_omniverse_loader(config, mode, batch_size=1, **kwargs):
    if mode=="train":
        dataloader=DataLoader(OmniverseObject(config.data_root,split='train'),batch_size=batch_size,**kwargs)

    else:
        dataloader=DataLoader(OmniverseObject(config.data_root,split='test'),batch_size=batch_size,**kwargs)
    return dataloader